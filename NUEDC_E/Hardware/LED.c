...
void LED_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1 | GPIO_Pin_2|GPIO_Pin_5|GPIO_Pin_8;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA, &GPIO_InitStructure);						//将PA1和PA2引脚初始化为推挽输出
	
	/*设置GPIO初始化后的默认电平*/
	GPIO_SetBits(GPIOA, GPIO_Pin_1 | GPIO_Pin_2|GPIO_Pin_5|GPIO_Pin_8);				//设置PA1和PA2引脚为高电平
}
...
void LED5_Turn(void)
{
	...
	{
		GPIO_SetBits(GPIOA, GPIO_Pin_5);					//则设置PA1引脚为高电平
	}
	else													//否则，即当前引脚输出高电平
	{
		GPIO_ResetBits(GPIOA, GPIO_Pin_5);					//则设置PA1引脚为低电平
	}
}
